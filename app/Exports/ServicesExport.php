...
class ServicesExport implements FromArray, WithEvents
{
    ...
    public function array(): array
    {
        ...
        $rows = [
            ...
            ['', '', '', '', '', '', ''], // Row 3
            ['SERVICES REPORT', '', '', '', '', '', ''],
            ['No.', 'Date Completed', 'Customer Name', 'Service', 'Category', 'Type', 'Price'],
        ];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getColumnDimension('C')->setWidth(30);  // Customer
                $sheet->getColumnDimension('D')->setWidth(30);  // Service
                $sheet->getColumnDimension('E')->setWidth(30);  // Category
                $sheet->getColumnDimension('F')->setWidth(25);  // Type
                $sheet->getColumnDimension('G')->setWidth(25);  // Price
                ...
                $sheet->setCellValue("G{$grandTotalRow}", $grandTotal);
                ...
                $sheet->getStyle("G{$grandTotalRow}")
                    ->getNumberFormat()
                    ->setFormatCode('₱#,##0.00');

                /* ================= BORDERS ================= */

                $sheet->getStyle("A4:G{$grandTotalRow}")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
